...
export default class Main extends React.Component {
  ...
  render() {
    return (
      <ArticleList articleList={this.state.list} hasMoreText={this.state.hasMoreText} isSigin fetchData={(i, cb) => {
  ...
  fetchDatas(page, cb) {
    ...
    fetchData(`http://${config.ip}:${config.port}/getAllArticle`, { method: 'post', data: {
      page
    }}).then((res) => {
        cb && cb();
        if(res.status === 0 ) {
